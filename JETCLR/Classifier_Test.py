...
# initialise logfile
logfile = open( "/remote/gpu05/rueschkamp/outputs_from_queue/CLR/Classifier/Class_logfile", "a" )
...
#Loading Model -------------________-----__---____--_-___------__--
loading_model = True
if loading_model :
    ...
    state_dict = torch.load("/remote/gpu05/rueschkamp/outputs_from_queue/CLR/Big_Model_colfil.pt")
...
data_LCT_train = My_Dataset(args.tr_dat_path,args.val_dat_path,args.test_dat_path,"LCT_train",number_of_jets= 1e4)
data_LCT_test = My_Dataset(args.tr_dat_path,args.val_dat_path,args.test_dat_path,"LCT_test",number_of_jets= 1e4/3)
...
def train(epochs, model, criterion, train_dataloader):
    ...
    for epoch in range(epochs):
        ...
        tl1 = time.time()
        print(f"Epoch {epoch}: loss={epoch_loss:.4f} time taken{round(tl1-tl0,2)}", flush=True, file=logfile)
...
tms0 = time.time()
torch.save(linear_model.state_dict(),"/remote/gpu05/rueschkamp/outputs_from_queue/CLR/Classifier/LCT_Model.pt")
tms1 = time.time()
...
plt.legend()
plt.savefig("/remote/gpu05/rueschkamp/outputs_from_queue/CLR/Classifier/Loss.pdf",format="pdf")
...
plt.title(f'Receiver operating characteristic curve, 700 epochs (both), 1e5 jets')
plt.legend(loc="lower right")
plt.savefig("/remote/gpu05/rueschkamp/outputs_from_queue/CLR/Classifier/ROC.pdf",format= "pdf" )